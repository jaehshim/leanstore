...
struct WiredTigerDB {
   ...
   //                             std::to_string(u64(FLAGS_dram_gib * 1024)) + "M");
      std::string config_string("create, log=(enabled), checkpoint=(wait=60), cache_size=" +
                                std::to_string(u64(FLAGS_dram_gib * 1024)) + "M");
...
struct WiredTigerAdapter : public Adapter<Record> {
   ...
   WiredTigerAdapter(WiredTigerDB& map) : map(map)
   {
      ...
      int ret = map.session->create(map.session, table_name.c_str(), "key_format=S,value_format=S");
      error_check(ret);
   }
   // -------------------------------------------------------------------------------------
